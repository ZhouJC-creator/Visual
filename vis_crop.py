...
from DataAugmentation.Crop import attention_crop
...
model_weight = "RiceLeaf_15_0.99286.bin"
imgName = "Brown_Spot.jpeg"
...
logits, att_mat, _, _, _ = model(x)

head_select_rate = torch.full((1, 12), 128).squeeze(0) / (128 * 12)
att_mat = att_mat[10]
# crop_imgs, _,_ = attention_crop(att_mat, x,head_select_rate,(128 * 12))
crop_imgs= attention_crop(att_mat, x)
...
Image.fromarray(crop_img).save(f"./output/DA/{dataset_name}/{imgName}_normal_croped_single.jpg")  # 保存裁剪后的图片
...
# 保存展示图为文件
plt.tight_layout()
plt.savefig(f"./output/DA/{dataset_name}/{imgName}_normal_croped.jpg")  # 保存对比图
...
#
#     # 保存图片
#     im.save("original_image.png")  # 保存原图
#     # Image.fromarray(crop_img).save("cropped_image.png")  # 保存裁剪后的图片
#
#     # 创建图像展示
#     fig, axes = plt.subplots(1, 2, figsize=(10, 5))
#
...
#     # 保存展示图为文件
#     plt.tight_layout()
#     plt.savefig(f"./output/crop/{imgName}_layer{i+1}.jpg")  # 保存对比图
