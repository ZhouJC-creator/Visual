...
logits, att_mat,heads_select_rate, heads_select_num, heads_select_index = model(x.unsqueeze(0))
# logits, att_mat,_ = model(x.unsqueeze(0))
...
for i, v in enumerate(joint_attentions):
    ...
    if i==0:
        im.save(f"./output/headHeatMap/{model_name}/{dataset_name}/{imgName.split('.')[0]}_original.jpg")

    # Save the attention heatmap overlay
    overlay_rgb = cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB for consistency

    # Save overlay images
    plt.imsave(f"./output/headHeatMap/{model_name}/{dataset_name}/{imgName.split('.')[0]}_Head_{i + 1}.jpg", overlay_rgb)  # Save overlay as RGB

    # Optionally display results using matplotlib
    # fig, (ax1, ax2) = plt.subplots(ncols=2, figsize=(16, 16))
    # ax1.set_title('Original')
    # ax2.set_title(f'Attention Map - Head {i + 1}')
    # ax1.imshow(im)
    # ax2.imshow(overlay_rgb)
    # plt.savefig(f"./output/headHeatMap/{model_name}/{dataset_name}/{imgName.split('.')[0]}_vis_Head_{i + 1}.jpg")
